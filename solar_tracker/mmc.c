...
//Modified for Mega328P - Arduino Uno
#define SS     (1<<PB0) // Remember! SD_CS on Wireless SD Shield connects to Pin 4 (PD4) on Arduino Uno
#define MOSI   (1<<PB2)
#define MISO   (1<<PB3)
#define SCK    (1<<PB1)
...
#define FCLK_SLOW() SPCR |= (1<<SPE) | (1<<MSTR) | (1<<SPR1) | (1<<SPR0) // Set slow clock (100-400kHz) 16000000 / 64 = 250KHz
#define FCLK_FAST() SPCR |= (1<<SPE) | (1<<MSTR) | (1<<SPR0) // Set fast clock (2 MHz) 16000000 / 8 = 2MHz
...
void power_on (void)
{
   ...
   DDRB  |= SS;
	...
	SPCR |= (1<<SPE) | (1<<MSTR) | (1<<SPR1) | (1<<SPR0) ;	/* Enable SPI function in mode 0 CLK/64*/
...
//  Following remmed out as not supported
